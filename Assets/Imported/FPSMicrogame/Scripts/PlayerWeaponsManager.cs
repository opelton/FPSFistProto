...
public class PlayerWeaponsManager : MonoBehaviour {
    ...
    [Tooltip("Overload firing angle"), SerializeField]
    Transform weaponShotAngle;
    ...
    public Vector3 shotPosition {
        get {
            // shot should come from active weapon
            var activeWeapon = GetActiveWeapon();
            if (activeWeapon != null)
                return activeWeapon.weaponMuzzle.position;

            return lookPosition;
        }
    }

    // saved result of this frame's raycast down the gun barrel
    RaycastHit _weaponRaycastInfo;
    public RaycastHit WeaponRaycastInfo { get { return _weaponRaycastInfo; } }

    // return true if weapon raycast was true this frame
    bool _weaponRaycastHit = false;
    public bool WeaponraycastHit { get { return _weaponRaycastHit; } }
    ...
    bool _isHolstering = false;
    float _timeStartedHolstering;
    bool _aimBlocked = false;
    ...
    private void Start() {
        ...
        // Update weapon raycast immediately so the info is always available
        UpdateWeaponRaycastInfo();
    }
    ...
    public void LowerWeapon() {
        ...
        _isHolstering = true;
        _timeStartedHolstering = Time.time;
    }
    ...
    public void SetAimBlock(bool blocked) {
        _aimBlocked = blocked;
    }

    bool CanAim() {
        return m_WeaponSwitchState == WeaponSwitchState.Up && !_aimBlocked;
    }
    ...
    private void Update() {
        ...
        if (activeWeapon && m_WeaponSwitchState == WeaponSwitchState.Up && !_isHolstering) {
            // handle aiming down sights
            isAiming = CanAim() && m_InputHandler.GetAimInputHeld();
        ...
        if (activeWeapon && _weaponRaycastHit) {
            if (WeaponRaycastInfo.collider.GetComponentInParent<EnemyController>()) {
                isPointingAtEnemy = true;
            }
        }
    }

    void UpdateWeaponRaycastInfo() {
        _weaponRaycastHit = CheckWeaponRaycast(out _weaponRaycastInfo);
    }

    bool CheckWeaponRaycast(out RaycastHit hit) {
        return Physics.Raycast(shotPosition, shotDirection, out hit, 1000);
    }
    ...
    // Updates weapon position and ability to fire when holstering weapon
    void UpdateWeaponHolstering() {
        if (_isHolstering) {
            float holsterTimeRatio = 0f;
            if (weaponSwitchDelay == 0f) {
                holsterTimeRatio = 1f;
            } else {
                // use the same timing as swapping weapons
                holsterTimeRatio = Mathf.Clamp01((Time.time - _timeStartedHolstering) / weaponSwitchDelay);
            }

            // unholster the lowered weapon
            if (m_WeaponSwitchState == WeaponSwitchState.Down) {
                m_WeaponMainLocalPosition = Vector3.Lerp(downWeaponPosition.localPosition, defaultWeaponPosition.localPosition, holsterTimeRatio);
            }
            // lower weapon
            else {
                m_WeaponMainLocalPosition = Vector3.Lerp(defaultWeaponPosition.localPosition, downWeaponPosition.localPosition, holsterTimeRatio);
            }

            // transition to the next state
            if (holsterTimeRatio >= 1) {
                var wasDown = m_WeaponSwitchState == WeaponSwitchState.Down;
                m_WeaponSwitchState = wasDown ? WeaponSwitchState.Up : WeaponSwitchState.Down;
                _isHolstering = false;
            }
        }
    }
    ...
    public bool AddWeapon(WeaponController weaponPrefab) {
        ...
        for (int i = 0; i < m_WeaponSlots.Length; i++) {
            ...
            if (m_WeaponSlots[i] == null) {
                ...
                int layerIndex = GetWeaponLayerIndex();
    ...
    public int GetWeaponLayerIndex() {
        return Mathf.RoundToInt(Mathf.Log(FPSWeaponLayer.value, 2)); // This function converts a layermask to a layer index
    }
